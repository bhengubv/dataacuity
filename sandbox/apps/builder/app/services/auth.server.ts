  ...
  env.BUILDER_ORIGIN
    ...
    : env.DEPLOYMENT_ENVIRONMENT === "production"
      ...
      : env.DEPLOYMENT_ENVIRONMENT === "staging" ||
        ...
        : `https://sandbox.dataacuity.co.za:${env.PORT || 5173}`;
  ...
  const oidc = new OAuth2Strategy(
    {
      ...
      authorizationEndpoint: `${env.OIDC_ISSUER}/protocol/openid-connect/auth`,
      tokenEndpoint: `${env.OIDC_ISSUER}/protocol/openid-connect/token`,
    ...
    async ({ tokens, request }) => {
      ...
      const userInfoResponse = await fetch(
        `${env.OIDC_ISSUER}/protocol/openid-connect/userinfo`,
        {
          headers: {
      ...
      // Map OIDC profile to match the expected format
      const profile = {
        id: userInfo.sub,
      ...
      return strategyCallback({ profile: profile as any, request });
    }
  );
